...
<table class="table-auto w-full text-center border text-sm">
  ...
  <tbody>
    ...
    <tr class="border-t">
      ...
      <td class="px-2 py-1">
        <input type="date" bind:value={servicio.checkin} min={fechaInicio} max={servicio.checkout || fechaFin}
          on:change={()=> validarFechas(servicio, index)}
        class="rounded border px-2 py-1 text-sm"
        />
      </td>
      <td class="px-4 py-2">
        <input type="date" bind:value={servicio.checkout} min={servicio.checkin || fechaInicio} max={fechaFin}
          on:change={()=> validarFechas(servicio, index)}
        class="rounded border px-2 py-1 text-sm"
        />
      </td>
